...
const challengeHash = (userId, purpose, code) => hash(purpose === 'athlete-password' ? userId + ':' + purpose + ':' + code : code);
async function createChallenge(userId, purpose = 'password', code = secret(), ttl = 3600000) {
  ...
  await db.runAsync('INSERT INTO RecoveryChallenges(token_hash,usuario_id,purpose,expires) VALUES(?,?,?,?)', [challengeHash(userId, purpose, code), userId, purpose, Date.now()+ttl]);
  });
  return code;
}
async function resetWithChallenge(userId, purpose, code, password) {
  ...
  await ensureSecuritySchema(db);
  const tokenHash = challengeHash(userId, purpose, code);
  ...
  await db.transaction(async () => {
  const user = await db.getAsync('SELECT ativo,activation_pending FROM Usuarios WHERE id=?', [userId]);
  if (!user || user.ativo !== 1 || (user.activation_pending && purpose !== 'activation')) throw httpError(400, 'Código inválido ou expirado.');
  ...
  if (purpose === 'activation') await db.runAsync('UPDATE Usuarios SET activation_pending=0 WHERE id=?', [userId]);
  await db.runAsync('UPDATE RecoveryChallenges SET used=1 WHERE usuario_id=?', [userId]);
