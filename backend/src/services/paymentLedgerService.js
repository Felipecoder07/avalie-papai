...
async function recompute(reservationId) {
  ...
  const paid=Math.round(row.paid*100),total=cents(row.valor_total,{zero:true});
  let state=paid>=total?'Pago':paid>0?'Parcial':'Pendente';
  if(row.status==='Cancelada') state=paid>0?'Cancelado (Pendente Estorno)':'Estornado';
  const bookingState=row.status==='Cancelada'?'Cancelada':paid>=total?'Confirmada':row.status;
  await db.runAsync('UPDATE Reservas SET status_pagamento=?,status=? WHERE id=?',[state,bookingState,row.id]);
  return {totalPago:paid/100,saldoDevedor:Math.max(0,total-paid)/100,novoStatus:state};
}
async function scopeReservations(reserva) {
  return reserva.grupo_id
...
async function settle(gatewayRef,payload={}) {
  ...
  return db.transaction(async()=>{
    ...
    const amount=cents(tx.valor);
    if(payload.valor_pago!==undefined && cents(payload.valor_pago)!==amount) throw httpError(400,'Valor confirmado divergente da cobrança.');
    ...
    for(const r of reservations) {
      ...
      const allocation=Math.min(left,Math.max(0,cents(r.valor_total,{zero:true})-Math.round(paid.total*100)));
      if(allocation>0) {
        ...
        await db.runAsync('INSERT INTO Pagamentos(reserva_id,valor,metodo,registrado_por) VALUES(?,?,?,NULL)',[r.id,allocation/100,method]);
        left-=allocation;
      }
      await recompute(r.id);
    }
    // An excess/late payment remains a real credit and is flagged for reconciliation.
    if(left>0 || reservations.some(r=>r.status==='Cancelada')) {
...
async function reverse(gatewayRef,refundedAmount) {
  ...
  return db.transaction(async()=>{
    ...
    const desired=cents(refundedAmount,{zero:true});
    ...
    for(const a of allocations) {
      ...
      if(deducted) {
        await db.runAsync('INSERT INTO Pagamentos(reserva_id,valor,metodo) VALUES(?,?,?)',[a.reserva_id,-deducted/100,'Estorno']);
